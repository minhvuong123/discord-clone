...
export default async function handler(
  ...
  if (req.method !== "DELETE" && req.method !== "PATCH") {
    return res.status(405).json({ error: "Method not allwed" })
  }
  ...
  try {
    ...
    if (!profile) {
      return res.status(400).json({ error: "Unauthorized - Profile" })
    }
    ...
    if (!canModify) {
      return res.status(404).json({ error: "Unauthorized - canModify" })
    }
    ...
    if (req.method === "PATCH") {
      if (!isMessageOwner) {
        return res.status(404).json({ error: "Unauthorized - isMessageOwner" })
      }
